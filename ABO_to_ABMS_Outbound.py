...
def prepare_payload(ids):
    return [{'Id': record_id, 'Significant_Fields_Updated__c': False} for record_id in ids]

def bulk_update(bulk, object_name, records):
    job = bulk.create_update_job(object_name, contentType='JSON')
    for chunk in chunked_iterable(records, CHUNK_SIZE):
        payload = json.dumps(chunk)
        bulk.post_batch(job, IteratorBytesIO(payload.encode('utf-8')))
    bulk.close_job(job)
    ...
    for idx, contact in enumerate(contacts):
        ...
        if medical_licenses and 'records' in medical_licenses:
            ...
            if licenses:
                ...
                for license_idx, license_record in enumerate(licenses):
                    ...
                    license_id = license_record.get('Id', f"Unknown-{license_idx}")
                    logger.debug(f"Processing medical license {license_idx+1}/{len(licenses)}: {license_id}")
                    MEDICAL_LICENSE_IDS.append(license_id)
        ...
        for placeholder, field_name in field_mapping.items():
            if placeholder.startswith("{{Contact."):
                ...
                #     ]))
                if "LastModifiedDate" in placeholder and value:
                    try:
                        value_str = str(value)[:10]
                    except Exception as e:
                        logger.warning(f"Error converting value to date: {value} — {e}")
                        value_str = ''
...
def generate_salesforce_xml(xml_file_name):
    ...
    try:
        ...
        upload_xml_to_library(sf, pretty_xml, title, library_id)
        logger.info(f"XML file uploaded successfully.")
        
        contact_failed = False
        license_failed = False
        contact_records = []
        license_records = []
        contact_exception = None
        license_exception = None
        # Update Contact records
        if CONTACT_IDS:
            try:
                contact_records = prepare_payload(CONTACT_IDS)
                bulk_update(bulk, 'Contact', contact_records)
                logger.info(f"Successfully updated {len(CONTACT_IDS)} Contact records.")
            except Exception as e:
                contact_failed = True
                contact_exception = e
                logger.error(f"Failed to update Contact records: {e}", exc_info=True)

        # Update Medical License records
        if MEDICAL_LICENSE_IDS:
            try:
                license_records = prepare_payload(MEDICAL_LICENSE_IDS)
                bulk_update(bulk, 'Medical_License__c', license_records)
                logger.info(f"Successfully updated {len(MEDICAL_LICENSE_IDS)} Medical License records.")
            except Exception as e:
                license_failed = True
                license_exception = e
                logger.error(f"Failed to update Medical License records: {e}", exc_info=True)
        
        if contact_failed or license_failed:
            payload = str(contact_records)+str(license_records)
            response = str(contact_exception)+str(license_exception)
            integration_log = {
                'Status_Code__c': '500',
                'Message__c': 'Error Updating Contact/MedicalLicenses',
                'Request_Payload__c': payload,
                'Response_Payload__c': response,
                'Log_Type__c': 'Python Integration'
            }
            try:
                sf.Integration_Log__c.create(integration_log)
...
def download_xml():
    ...
    try:
        generate_salesforce_xml(xml_file_name)
...
def health_check():
    ...
    return "ABO to ABMS Outbound API is running"
